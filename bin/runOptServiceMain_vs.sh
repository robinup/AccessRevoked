...
# The difference of this script to runOptServiceBackup.sh is that this runs a different jar file 
# that have the vertica engine enabled

root=$(dirname $0)/..

cp=".:./../hbaseconfig:$root/dist/TapjoyOptService_optmain.jar"
...
exec java -server $gc $ja -cp $cp com.tapjoy.opt.OptimizationService --production y --productionS3 y --databaseServer main --algorithms 101
